...
if(isset($_POST['Submit'])) {
	...
	if (isset($_FILES['image']['name'])) {
	  if (($userfile_type == "image/jpeg") || ($userfile_type == "image/jpg")) {
	    ...
	    echo "<br/></div>";
	    $url = "controller_info.php?lang=".$lang."&line=".$line."&inv=".$inv."&sys=".$sys."&id=".$id;
	    redir($url,'2');
	  } else {
	echo "<div class='w94 pt10'>";
	get_error($lang,'1040');
	...
	echo "
	<form method=\"POST\" action=\"".$self_path."?lang=".$lang."&line=".$line."&inv=".$inv."&sys=".$sys."&id=".$id."\" enctype=\"multipart/form-data\">
...
function photodel(line,inv) {
   ...
   success: function(){
     ...
     var delay = 1000;
     setTimeout(function(){ 
	   window.location.href = 'controller_photo_update.php?lang=<?php echo $lang; ?>&line=<?php echo $line; ?>&inv=<?php echo $inv; ?>&id=<?php echo $id; ?>&sys=<?php echo $sys; ?>'; }, delay);
   }
 });
    return false;
}
