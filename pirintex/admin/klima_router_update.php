...
if(isset($_POST['submit'])) {
	...
	$e_router_name = mysql_prep($_POST['router_name']);
	$e_router_sname = mysql_prep($_POST['router_sname']);
	...
	$check = check_router_id_and_name($e_router_name,$e_id);
    if (($e_id != NULL) && ($check == 0)) {
	    mysql_query("SET NAMES utf8");
		$query = "UPDATE `klima_lines` SET `router_name`='$e_router_name', `router_sname`='$e_router_sname', `data_file`='$e_data_file', `ip_address`='$e_ip_address', `port`='$e_port', `ex_table`='$e_ex_table', `org`='$e_org' WHERE `id`='$e_id'";
            $result = mysql_query($query);
            confirm_query($result);
            if ($result) {
		        echo "<div class=\"w94 pt10\">";
		        get_success($lang,'2020');
		        echo "<br/></div>";
				insert_log($lang,$user_settings['user_name'],'warning','ad306',$e_router_name);
				$url = "klima_routers.php?lang=".$lang;
				redir($url,'3');
            } else {
	echo "<div class=\"w94 pt10\">";
	get_error($lang,'1043');
