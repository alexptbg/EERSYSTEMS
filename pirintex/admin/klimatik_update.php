...
if(isset($_POST['submit'])) {
	...
	$check = check_klima_inv_and_id($inv,$id);
    if ($inv == mysql_prep($_POST['inv']) && ($check == 0)) {
	    $query = "UPDATE `klimatik` SET `inv`='$inv', `router`='$router', `main_line`='$main_line', `line`='$linex', `date`='$date', `rtu`='$rtu', `addr`='$addr', `where`='$where', `name`='$name', `desc`='$desc', `org`='$org', `plant`='$plant', `floor`='$floor' WHERE `id`='$id' AND `router`='$router'";
            $result = mysql_query($query);
            confirm_query($result);
            if ($result) {
		        echo "<div class='w94 pt10'>";
		        get_success($lang,'2022');
		        echo "<br/></div>";
				$obs = $router."|".$inv;
				insert_log($lang,$user_settings['user_name'],'warning','ad320',$obs);
				$url = "klimatiki.php?lang=".$lang."&krouter=".$krouter."&inv=".$inv;
				redir($url,'3');
            } else {
	echo "<div class=\"w94 pt10\">";
	get_error($lang,'1048');
