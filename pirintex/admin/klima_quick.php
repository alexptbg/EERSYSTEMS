...
$query = "SELECT `router_name`,`ip_address`,`port`,`data_file` FROM `klima_lines`";
	          ...
	          <script type=\"text/javascript\" charset=\"utf-8\">
                  $(function() {
                      $('select#ex_".$rand."').switchify().data('switch').bind('switch:slide', function(e, type) {
                          if (type == 'on') {
                              var postData = { 'action' : 'on', 'kline' : '".$klines['router_name']."' }; 
                              $.ajax({
                                  url: 'klima_con_handle.php',
                                  type: 'post',
                                  data: postData,
                                  success: function(result){ }
                              });
	                      } else {
                              var postData = { 'action' : 'off', 'kline' : '".$klines['router_name']."' }; 
                                  $.ajax({
                                      url: 'klima_con_handle.php',
                                      type: 'post',
                                      data: postData,
				...
				<div class='row-form clearfix'>
				    <div class='span4 tar'>".$klines['router_name'].":</div>
					<div class='span8'>";
                    $datafile = "../data/{$klines['data_file']}";
