        ...
        <div class="content">
            ...
            <div class="workplace">
                <div class="row-fluid">
                    ...
                    <div class="span12">                    
                        <div class="head clearfix">
                            <i class="fa fa-plus fa-1x"></i><span><?php echo get_lang($lang, 'info1'); ?></span> 
                        </div>
...
if(isset($_POST['submit'])) {
	...
	$line = mysql_prep($_POST['line']);
    $date = mysql_prep($_POST['from']);
    $rtu = mysql_prep($_POST['rtu']);
    $addr = mysql_prep($_POST['addr']);
    $where = mysql_prep($_POST['where']);
    $name = mysql_prep($_POST['name']);
    $desc = mysql_prep($_POST['desc']);
    $org = mysql_prep($_POST['org']);
    $plant = mysql_prep($_POST['plant']);
    $floor = mysql_prep($_POST['floor']);
    $check = get_check_klimatik_db($router,$inv);
    if ($check == NULL) {
		mysql_query("SET NAMES utf8");
        $query = "INSERT INTO `klimatik` (`inv`, `router`, `main_line`, `line`, `date`, `rtu`, `addr`, `where`, `name`, `desc`, `org`, `plant`, `floor`) 
		          VALUES ('$inv', '$router', '$main_line', '$line', '$date', '$rtu', '$addr', '$where', '$name', '$desc', '$org', '$plant', '$floor')";
            $result = mysql_query($query);
            confirm_query($result);
            if ($result) {
		        echo "<div class=\"w94 pt10\">";
		        get_success($lang,'2023');
		        echo "<br/></div>";
				$obs = $router."|".$inv;
				insert_log($lang,$user_settings['user_name'],'warning','ad321',$obs);
				$url = "klimatiki.php?lang=".$lang."&krouter=".$router."&inv=".$inv;
				redir($url,'3');
            } else {
	echo "<div class=\"w94 pt10\">";
	get_error($lang,'1049');
