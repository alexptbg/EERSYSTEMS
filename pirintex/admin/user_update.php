...
if(isset($_POST['submit'])) {
    ...
    $e_s_admin = mysql_prep($_POST['s_admin']);
    $e_level = mysql_prep($_POST['level']);
    ...
    $e_info = mysql_prep($_POST['info']);	
		if ($e_user_name == mysql_prep($_POST['user_name'])) {
			...
			$query = "UPDATE `users` SET `user_name`='$e_user_name', `s_admin`='$e_s_admin', `level`='$e_level', `status`='$e_status', `first_name`='$e_first_name', `last_name`='$e_last_name', `email`='$e_email', `s_email`='$e_s_email' , `phone`='$e_phone', `info`='$e_info' WHERE `user_name`='$e_user_name'";
            $result = mysql_query($query);
            confirm_query($result);
            if ($result) {
		        echo "<div class='w94 pt10'>";
		        get_success($lang,'2006');
		        echo "<br/></div>";
				insert_log($lang,$user_settings['user_name'],'warning','ad105',$e_user_name);
