...
if(isset($_POST['submit'])) {
    $add_user_name = mysql_prep($_POST['user_name']);
    $add_password = mysql_prep($_POST['password']);
    $add_password2 = mysql_prep($_POST['password2']);
    $add_first_name = mysql_prep($_POST['first_name']);
    $add_last_name = mysql_prep($_POST['last_name']);
    $add_email = mysql_prep($_POST['email']);
    $add_s_email = mysql_prep($_POST['s_email']);
    $add_s_admin = mysql_prep($_POST['s_admin']);
    $add_level = mysql_prep($_POST['level']);
    $add_status = mysql_prep($_POST['status']);
    $add_phone = mysql_prep($_POST['phone']);
    $add_info = mysql_prep($_POST['info']);	
    $check = check_user_name($add_user_name);
	$check_mail = check_user_mail($add_email);
    if (($check == NULL) && ($check_mail == NULL)) {
		$passwd1 = md5($add_password); $passwd2 = md5($add_password2);
		if ($passwd1 == $passwd2) {
			mysql_query("SET NAMES utf8");
			$query = "INSERT INTO `users` (`user_name`, `h_password`, `s_admin`, `level`, `status`, `first_name`, `last_name`, `email`, `s_email`, `phone`, `info`) 
		              VALUES ('$add_user_name', '$passwd2', '$add_s_admin', '$add_level', '$add_status', '$add_first_name', '$add_last_name', '$add_email', '$add_s_email', '$add_phone', '$add_info')";
            $result = mysql_query($query);
            confirm_query($result);
            if ($result) {
		        echo "<div class='w94 pt10'>";
		        get_success($lang,'2005');
		        echo "<br/></div>";
				insert_log($lang,$user_settings['user_name'],'warning','ad102',$add_user_name);
                ...
                //echo "User add Failed ! </br>" . mysql_error() . " ";
            }
		} else {
		    echo "<div class='w94 pt10'>";
		    get_error($lang,'1022');
		...
		echo "</div><div class='footer'><button class='btn btn-default' type='button' onClick='javascript: history.go(-1); return false;'>
		      <i class='fa fa-arrow-left'></i>&nbsp; ". get_lang($lang, 'Back')."</button></div>";
		//echo "user or mail exists in db";
	}
} else {
	echo "<div class='w94 pt10'>";
	get_error($lang,'1000');
