...
if(isset($_POST['submit'])) {
	$i_router_name = mysql_prep($_POST['router_name']);
	$i_router_sname = mysql_prep($_POST['router_sname']);
	...
	$check = check_router_name($i_router_name);
    if ($check == NULL) {
	    mysql_query("SET NAMES utf8");
		$query = "INSERT INTO `klima_lines` (`router_name`, `router_sname`, `data_file`, `ip_address`, `port`, `ex_table`, `org`) VALUES ('$i_router_name', '$i_router_sname', '$i_data_file', '$i_ip_address', '$i_port', '$i_ex_table', '$i_org')";
            $result = mysql_query($query);
            confirm_query($result);
            if ($result) {
		        echo "<div class=\"w94 pt10\">";
		        get_success($lang,'2021');
		        echo "<br/></div>";
				insert_log($lang,$user_settings['user_name'],'warning','ad305',$i_router_name);
