        ...
        <div class="content">
            ...
            <div class="workplace">
                <div class="row-fluid">
                    ...
                    <div class="span12">                    
                        <div class="head clearfix">
                            <i class="fa fa-edit fa-1x"></i><span><?php echo get_lang($lang, 'ad303'); ?></span>                          
                        </div>
                        <div class="block-fluid">                        
<?php
if(isset($_POST['submit'])) {
	$e_id = mysql_prep($_POST['id']);
	$e_line_name = mysql_prep($_POST['line_name']);
	$e_line_sname = mysql_prep($_POST['line_sname']);
	$e_data_file = mysql_prep($_POST['data_file']);
	$e_ip_address = mysql_prep($_POST['ip_address']);
	$e_port = mysql_prep($_POST['port']);
	$e_alarm = mysql_prep($_POST['alarm']);
	$e_red = mysql_prep($_POST['red']);
	$e_ex_alarm = mysql_prep($_POST['ex_alarm']);
	$e_ex_red = mysql_prep($_POST['ex_red']);
	$e_ex_table = mysql_prep($_POST['ex_table']);
	$e_org = mysql_prep($_POST['org']);
	$e_plant = mysql_prep($_POST['plant']);
	$e_floor = mysql_prep($_POST['floor']);
    $e_lt_s = $_POST['lt_s'];
    $e_lt_start = $_POST['lt_start'];
    $e_lt_stop = $_POST['lt_stop'];
    $e_mod_s = $_POST['mod_s'];
    $e_mod_start = $_POST['mod_start'];
    $e_mod_stop = $_POST['mod_stop'];
    $e_time_s = $_POST['time_s'];
    $e_time_start = $_POST['time_start'];
    $e_time_stop = $_POST['time_stop'];
    if ($e_lt_s == 'on') { $lt_s = 1; } else { $lt_s = 0; }
    if ($e_mod_s == 'on') { $mod_s = 1; } else { $mod_s = 0; }
    if ($e_time_s == 'on') { $time_s = 1; } else { $time_s = 0; }
	$check = check_line_id_and_name($e_line_name,$e_id);
    if (($e_id != NULL) && ($check == 0)) {
	    mysql_query("SET NAMES utf8");
		$query = "UPDATE `lines` SET `line_name`='$e_line_name', `line_sname`='$e_line_sname', `data_file`='$e_data_file', `ip_address`='$e_ip_address', `port`='$e_port', `alarm`='$e_alarm', `red`='$e_red', `ex_alarm`='$e_ex_alarm', `ex_red`='$e_ex_red', `ex_table`='$e_ex_table', `org`='$e_org', `plant`='$e_plant', `floor`='$e_floor', `lt_s`='$lt_s', `lt_start`='$e_lt_start', `lt_stop`='$e_lt_stop', `mod_s`='$mod_s', `mod_start`='$e_mod_start', `mod_stop`='$e_mod_stop', `time_s`='$time_s', `time_start`='$e_time_start', `time_stop`='$e_time_stop' WHERE `id`='$e_id'";
            $result = mysql_query($query);
            confirm_query($result);
            if ($result) {
		        echo "<div class='w94 pt10'>";
		        get_success($lang,'2008');
		        echo "<br/></div>";
				insert_log($lang,$user_settings['user_name'],'warning','ad110',$e_line_name);
